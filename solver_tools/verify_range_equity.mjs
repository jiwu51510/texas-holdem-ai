...
// 运行验证
async function main() {
    console.log('验证范围vs范围胜率计算');
    console.log('枚举所有OOP vs IP组合对');
    console.log(`\nOOP范围: ${OOP_RANGE}`);
    console.log(`IP范围: ${IP_RANGE}`);
    
    // Case 1: 公共牌 4c 2h 7s Kd 5c, OOP手牌 AcKs
    await calculateRangeVsRangeEquity('4c 2h 7s Kd 5c', 'AcKs');
